...
void test_scheduler(){
  ...
  if (task_hour_one) scheduled_task_free(task_hour_one);
  if (task_hour_two) scheduled_task_free(task_hour_two);
  if (task_hour_less) scheduled_task_free(task_hour_less);
  if (task_hour_greater) scheduled_task_free(task_hour_greater);
  if (task_day_one) scheduled_task_free(task_day_one);
  if (task_day_two) scheduled_task_free(task_day_two);
  if (task_day_before) scheduled_task_free(task_day_before);
  if (task_day_fifthteen) scheduled_task_free(task_day_fifthteen);
  if (task_month_tuesday) scheduled_task_free(task_month_tuesday);
  if (task_month_one) scheduled_task_free(task_month_one);
  if (task_month_two) scheduled_task_free(task_month_two);
  if (task_month_one_fifthteen) scheduled_task_free(task_month_one_fifthteen);
  if (task_month_two_fifthteen) scheduled_task_free(task_month_two_fifthteen);
  
  if (date) date_free(date);
  // Final memory check
  test_run_long("test_scheduler_heap_check", free_memory, ESP.getFreeHeap());
}