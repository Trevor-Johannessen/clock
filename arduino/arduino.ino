...
void setup() {
  ...
  Serial.println("Testing scheduler_delta_vector.");
  delta = scheduler_delta_vector(24, 1<<19, 9);
  Serial.printf("Passed scheduler_delta_vector! (%d)\n", (int)delta); // Should be 10
  delta = scheduler_delta_vector(24, 1, 9);
  Serial.printf("Passed scheduler_delta_vector! (%d)\n", (int)delta); // Should be 15 (i think)
  delta = scheduler_delta_vector(24, (1<<19) + 1, 9);
  Serial.printf("Passed scheduler_delta_vector! (%d)\n", (int)delta); // Should be 10
  delta = scheduler_delta_vector(24, 0, 2);
  Serial.printf("Passed scheduler_delta_vector! (%d)\n", (int)delta); // Should be 255

  Serial.println("Testing scheduler_nearest_bit.");
  delta = scheduler_nearest_bit(1<<5);
  Serial.printf("Passed scheduler_nearest_bit! (%d)\n", (int)delta); // Should be 5

  Serial.println("Testing scheduler_current_date.");
  date = scheduler_current_date();
  Serial.printf("Passed scheduler_current_date! (%d-%d-%d %d:%d:%d [%d])\n", date->year, (int)date->month, (int)date->day, (int)date->hour, (int)date->minute, (int)date->second, (int)date->weekday);

  Serial.println("Testing scheduler_delta (5 seconds)");
  Serial.printf("5 second task -> %uld\n", scheduler_delta(task));
}
